...
const sourceVideo = path.join(root, 'test-videos', 'portrait-9x16-sample.mp4');
const inputDirectory = path.join(root, 'smoke-unicode-input', '视频素材');
const inputPath = path.join(inputDirectory, '竖屏 测试-水印.mp4');
const outputDirectory = path.join(root, 'smoke-unicode-output', '导出 结果');
const outputPath = path.join(outputDirectory, '竖屏 测试-水印-clean.mp4');
const userDataDir = path.join(root, 'smoke-user-data', 'unicode-paths-smoke');

await rm(outputPath, { force: true });
await mkdir(inputDirectory, { recursive: true });
await mkdir(outputDirectory, { recursive: true });
await copyFile(sourceVideo, inputPath);
...
try {
  ...
  if (task?.inputPath !== inputPath) failures.push(`Unexpected inputPath: ${task?.inputPath}`);
  ...
  if (!task?.outputPath?.includes('导出 结果') || !task?.outputPath?.includes('竖屏 测试-水印-clean.mp4')) {
    failures.push(`Unicode output path was not preserved: ${task?.outputPath}`);
  }
