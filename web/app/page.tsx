...
function isSupported(file: File) {
  return ACCEPTED_TYPES.has(file.type) || /\.(png|jpe?g|webp)$/i.test(file.name);
}

export default function Home() {
  const [phase, setPhase] = useState<Phase>("idle");
  const [message, setMessage] = useState("拖入一张 Gemini 图片开始处理");
  ...
  async function handleFile(file?: File) {
    ...
    if (!isSupported(file)) {
      setPhase("error");
      setMessage("仅支持 PNG、JPG、JPEG 和 WEBP 图片。");
      return;
    }
  ...
  return (
    <main className="shell">
      ...
      <section className="workspace" id="top">
        <div className="intro">
          <p className="eyebrow">Gemini 图片清理工具</p>
          <h1>拖入图片，清理右下角可见水印。</h1>
          <p>无需安装，图片只在你的浏览器中处理。支持 PNG、JPG、JPEG、WEBP。</p>
        </div>
        ...
        <div className="tool-grid">
          <section className="upload-pane" aria-label="图片上传">
            ...
            >
              ...
              <h2>{phase === "processing" ? "正在处理" : "选择或拖入图片"}</h2>
              <p>PNG / JPG / JPEG / WEBP</p>
              <button className="primary" type="button" onClick={(event) => { event.stopPropagation(); chooseFile(); }}>选择图片</button>
              <input ref={inputRef} type="file" accept="image/png,image/jpeg,image/webp" onChange={onChange} />
            </div>
            <p className={`status ${phase}`}>{message}</p>
          </section>

          <section className="result-pane" aria-label="处理结果">
      ...
      <footer>仅用于你拥有或获授权修改的 Gemini 生成图片。视频请使用桌面版。</footer>
    </main>
  );
}
